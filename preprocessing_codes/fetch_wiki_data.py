...
def shelve_database():
    """ This method uses the python shelve objec to store the """

    # if making the shelve db file, better delete the existing, else
    # it will keep adding values to the existing one
    os.remove(db_file_name) if os.path.exists(db_file_name) else None
    db = shelve.open(db_file_name)
    try:
        # by updating the shelve instead of populating the values
        # one by one, it saves time by cutting down the disk access
        db.update(database)
    finally:
        db.close() 
